...
bottleInnerVolume = cylinderCuFt(
    bottleInnerCylinderHeight, bottleInnerRadius
) + truncatedConeCuFt(bottleInnerRadius, bottleMouthInnerRadius, bottleInnerConeHeight)
...
bottleOuterVolume = cylinderCuFt(
    bottleOuterCylinderHeight, bottleOuterRadius
) + truncatedConeCuFt(bottleOuterRadius, bottleMouthOuterRadius, bottleOuterConeHeight)
...
recipeStorage["bottle, glass"] = Recipe(
    "glassblower",
    (bottleWeight, "lb"),
    [("timber", bottleStopperCuFt)],
    [("baked clay", bottleGlassWeightOfClay), ("quicklime", bottleGlassWeightQl)],
    description="max cap. 1 pint 2 oz; "
    + str(bottleTotalHeight * 12)
    + " in. high plus 0.5 in stopper",
)


recipeStorage["pig iron"] = Recipe(
    "smelter", (1, "lb"), [("iron ore", 1), ("coal", 0.5), ("limestone", 0.25)]
)
semiGoods.append("pig iron")

recipeStorage["cast iron"] = Recipe(
    "smelter",
    (1, "lb"),
    # first the components which go into cast iron
    [("manganese ore", 0.06), ("nickel ore", 0.01), ("coal", 0.5), ("limestone", 0.25)],
    [("pig iron", 0.93)],
...
volumePewterIngot = (pewterTinProportion / densityTin) + (
    pewterCopperProportion / densityCopper
)
densityPewter = (
    1 / volumePewterIngot
)  # because it's a 1 lb ingot over whatever the volume turned out to be
# comes out to 469.3 lbs/cubic ft
# checked it for sanity and the density looks good for an alloy with this much copper.
recipeStorage["pewter"] = Recipe(
    "smelter",
    (1, "lb"),
    [
        ("tin ore", pewterTinProportion),
        ("copper ore", pewterCopperProportion),
        ("coal", Decimal(0.5)),
...
sterlingSilverProportionSilver = (
    Decimal(1) - sterlingSilverProportionCopper - sterlingSilverProportionZinc
)
densitySterlingSilver = (
    sterlingSilverProportionCopper * densityCopper
    + sterlingSilverProportionSilver * densitySilver
    + (sterlingSilverProportionZinc * densityZinc)
)
recipeStorage["sterling silver"] = Recipe(
    "smelter",
    (1, "lb"),
    [
...
volumeBronzeIngot = (bronzeTinProportion / densityTin) + (
    bronzeCopperProportion / densityCopper
)
densityBronze = 1 / volumeBronzeIngot
# comes out to 544.4 lb/cuft
recipeStorage["bronze"] = Recipe(
    "smelter",
    (1, "lb"),
    [
        ("tin ore", bronzeTinProportion),
        ("copper ore", bronzeCopperProportion),
        ("coal", Decimal(0.5)),
        ("limestone", Decimal(0.5)),
    ],
    [],
...
recipeStorage["brass"] = Recipe(
    "smelter",
    (1, "lb"),
    [
        ("zinc ore", brassZincProp),
        ("copper ore", brassCopperProp),
        ("coal", Decimal(0.5)),
        ("limestone", Decimal(0.5)),
    ],
    [],
...
volumeBellmetalIngot = (bellmetalTinProportion / densityTin) + (
    bellmetalCopperProportion / densityCopper
)
densityBellmetal = 1 / volumeBellmetalIngot
# comes out to 532.8 lb/cuft, almost the same as bronze but not quite
recipeStorage["bell metal"] = Recipe(
    "smelter",
    (1, "lb"),
    [
        ("tin ore", bellmetalTinProportion),
        ("copper ore", bellmetalCopperProportion),
        ("coal", Decimal(0.5)),
        ("limestone", Decimal(0.5)),
    ],
    [],
...
recipeStorage["wrought iron"] = Recipe(
    "smelter",
    (1, "lb"),
...
recipeStorage["blade hilt"] = Recipe(
    "carpenter",
    (hiltWeight, "lb"),
    [("timber", hiltCuFt)],
...
shortswordWeight = (
    getWeight("pommel") + getWeight("blade hilt") + (Decimal(2) * getWeight("blade"))
)
recipeStorage["shortsword"] = Recipe(
    "blacksmith",
    (shortswordWeight, "lb"),
    [],
...
longswordWeight = (
    getWeight("pommel") + getWeight("blade hilt") + (Decimal(3.5) * getWeight("blade"))
)
recipeStorage["longsword"] = Recipe(
    "blacksmith",
    (longswordWeight, "lb"),
    [],
...
greatswordWeight = (
    getWeight("pommel") + getWeight("blade hilt") + (Decimal(4.5) * getWeight("blade"))
)
recipeStorage["greatsword"] = Recipe(
    "blacksmith",
    (greatswordWeight, "lb"),
    [],
...
recipeStorage["husked cereal"] = Recipe("miller", (1, "lb"), [("cereal", 1)], [])
semiGoods.append("husked cereal")

recipeStorage["flour"] = Recipe(
    "miller",
    (1, "lb"),
    [],
    [("husked cereal", 1)],
...
recipeStorage["feed, cattle/horse"] = Recipe(
    "miller",
    (1, "lb"),
    [],
    [("husked cereal", 1)],
...
recipeStorage["bread"] = Recipe("baker", (1, "lb"), [("salt", 0.05)], [("flour", 0.7)])

recipeStorage["quicklime"] = Recipe(
    "potter",
    (1, "lb"),
...
recipeStorage["mature ewe"] = Recipe(
    "farmer",
    (90, "lb"),
...
recipeStorage["mutton sheep"] = Recipe(
    "farmer",
    (130, "lb"),
    [],
...
recipeStorage["sheep milk"] = Recipe(
    "farmer",
    (milkGallonWeight, "lb"),
    [],
...
recipeStorage["mutton"] = Recipe(
    "butcher", (1, "lb"), [], [("mutton sheep", Decimal(1 / sheepMeatWeight))]
)
...
recipeStorage["calf"] = Recipe(
    "farmer",
    (calfSlaughterWeight, "lb"),
...
recipeStorage["veal"] = Recipe("butcher", (1, "lb"), [], [("calf", 1 / vealPerCalf)])

# this is a guess
abomasumWeight = Decimal(5)
recipeStorage["abomasum"] = Recipe(
    "butcher",
    (abomasumWeight, "lb"),
    [],
...
recipeStorage["abomasum, cured"] = Recipe(
    "butcher",
    (abomasumWeight, "lb"),
    [("salt", Decimal(0.25))],
    [("abomasum", 1), ("vinegar", 1)],
)
# semiGoods.append("abomasum, cured")

# this is a guess
pintsRennetPerAbomasum = Decimal(2)
recipeStorage["rennet"] = Recipe(
    "butcher",
    (waterWeightOnePint, "lb"),
    [],
...
recipeStorage["cow"] = Recipe(
    "farmer",
    (cowSlaughterWeight, "lb"),
...
recipeStorage["beef"] = Recipe("butcher", (1, "lb"), [], [("cow", (1 / cowMeatWeight))])
...
recipeStorage["tallow"] = Recipe("chandler", (1, "lb"), [], [("suet", 1)])
...
recipeStorage["ashes"] = Recipe(
    "chandler", (1, "lb"), [("timber", ashesRequiredTimberCuFt)], []
)

# made by leaching ashes in water
recipeStorage["lye"] = Recipe("chandler", (1, "lb"), [], [("ashes", 1)])
...
recipeStorage["soap, hard"] = Recipe(
    "chandler",
    (weightOneBarSoap, "lb"),
...
recipeStorage["fleshy cowhide"] = Recipe(
    "butcher", (60, "lb"), [], [("cow", 1)], unit=(50, "sq ft")
)
semiGoods.append("fleshy cowhide")

recipeStorage["rawhide"] = Recipe(
    "tanner",
    (15, "lb"),
    [],
    [("fleshy cowhide", 1)],
...
recipeStorage["tanned cowhide"] = Recipe(
    "tanner",
    (getWeight("rawhide"), "lb"),
    [],
...
recipeStorage["holy symbol, plain, wooden"] = Recipe(
    "carpenter", (1, "lb"), [("timber", 0.02)], []
)

recipeStorage["holy symbol, plain, iron"] = Recipe(
    "blacksmith", (1, "lb"), [], [("wrought iron", 1)]
)
...
recipeStorage["separated ochre clay"] = Recipe(
    "potter", (1, "lb"), [], [("baked clay", 10)]
)
semiGoods.append("separated ochre clay")

# primary componenent of paints and of dyes
recipeStorage["pigment, red/yellow"] = Recipe(
    "dyer", (1, "lb"), [], [("separated ochre clay", 1)]
)
semiGoods.append("pigment, red/yellow")

# component of blue dye
recipeStorage["lapis lazuli, ground"] = Recipe(
    "potter", (1, "lb"), [("lapis lazuli", 1)]
)
semiGoods.append("lapis lazuli, ground")

recipeStorage["lapis lazuli, cleaned"] = Recipe(
    "potter",
    (1, "lb"),
    [],
...
recipeStorage["pigment, ultramarine"] = Recipe(
    "dyer", (1, "lb"), [], [("lapis lazuli, cleaned", 1)]
)
semiGoods.append("pigment, ultramarine")

recipeStorage["malted grain"] = Recipe(
    "brewer",
    (1, "lb"),
    [("cereal", 1)],
...
recipeStorage["roasted malt"] = Recipe("brewer", (1, "lb"), [], [("malted grain", 1)])
...
recipeStorage["barrel head"] = Recipe(
    "cooper", (barrelHeadWeight, "lb"), [("timber", barrelCuFt)], []
)
semiGoods.append("barrel head")

barrelHeadCircumference = barrelHeadRadius * 2 * Decimal(pi)
fatHoopCuFt = (
    barrelHeadCircumference
    * (Decimal(1.25) / Decimal(12))
    * (gauge16WireThicknessInches / Decimal(12))
)
fatHoopWeight = densityWroughtIron * fatHoopCuFt

recipeStorage["barrel hoop, fat"] = Recipe(
    "blacksmith", (fatHoopWeight, "lb"), [], [("wrought iron", fatHoopWeight)]
)
...
thinHoopCuFt = (
    barrelBodyCircumference
    * (Decimal(1) / Decimal(12))
    * (gauge16WireThicknessInches / Decimal(12))
)
thinHoopWeight = densityWroughtIron * thinHoopCuFt

recipeStorage["barrel hoop, thin"] = Recipe(
    "blacksmith", (thinHoopWeight, "lb"), [], [("wrought iron", thinHoopWeight)]
)
...
recipeStorage["barrel stave"] = Recipe(
    "cooper", (staveWeight, "lb"), [("timber", staveCuFt)], []
)
semiGoods.append("barrel stave")

# bringing it all together
barrelWeight = (
    (numStaves * staveWeight)
    + (4 * thinHoopWeight)
    + (2 * fatHoopWeight)
    + (2 * barrelHeadWeight)
)
recipeStorage["barrel"] = Recipe(
    "cooper",
    (barrelWeight, "lb"),
    [],
    [
        ("barrel stave", numStaves),
        ("barrel hoop, fat", 2),
        ("barrel hoop, thin", 4),
        ("barrel head", 2),
    ],
...
recipeStorage["ale, barrel"] = Recipe(
    "brewer",
    (weightWaterOneGal * gallonsPerBarrel + getWeight("barrel"), "lb"),
    [("cereal", aleCerealPerGallon * gallonsPerBarrel)],
    [
        ("malted grain", aleMaltPerGallon * gallonsPerBarrel),
        ("roasted malt", aleRoastedMaltPerGallon * gallonsPerBarrel),
        ("barrel", 1),
    ],
...
recipeStorage["ale, by the tankard"] = Recipe(
    "brewer",
    (waterWeightOnePint, "lb"),
    [],
...
recipeStorage["ale, bottled"] = Recipe(
    "brewer",
    (waterWeightOnePint + getWeight("bottle, glass"), "lb"),
    [],
...
recipeStorage["beer, barrel"] = Recipe(
    "brewer",
    (weightWaterOneGal * gallonsPerBarrel + getWeight("barrel"), "lb"),
    [
        ("cereal", beerCerealPerGallon * gallonsPerBarrel),
        ("hops", beerHopsPerGallon * gallonsPerBarrel),
    ],
...
recipeStorage["beer, by the tankard"] = Recipe(
    "brewer",
    (waterWeightOnePint, "lb"),
    [],
...
recipeStorage["beer, bottled"] = Recipe(
    "brewer",
    (waterWeightOnePint + getWeight("bottle, glass"), "lb"),
    [],
...
recipeStorage["vinegar"] = Recipe(
    "brewer",
    (weightPintVinegar + getWeight("bottle, glass"), "lb"),
    [],
    [("beer, bottled", 1)],
    unit=(1, "pint"),
...
recipeStorage["greasy wool"] = Recipe("farmer", (25, "lb"), [], [("mature ewe", 1)])
semiGoods.append("greasy wool")


recipeStorage["fuller's earth"] = Recipe(
    "potter",
    (1, "lb"),
    [],
    [("baked clay", 3)],
...
recipeStorage["scoured wool"] = Recipe(
    "fuller", (15, "lb"), [], [("fuller's earth", 1), ("greasy wool", 1)]
)
...
recipeStorage["clean wool"] = Recipe(
    "miller",
    (1, "lb"),
    [],
...
recipeStorage["yarn, wool"] = Recipe(
    "spinner",
    (1, "lb"),
    [],
    [("clean wool", 1)],
...
woolClothWeight = (yarnFtPerClothSqFt / getUnitSize("yarn, wool")) * getWeight(
    "yarn, wool"
)
recipeStorage["wool cloth"] = Recipe(
    "weaver",
    (woolClothWeight, "lb"),
    [],
...
# which is discarded
recipeStorage["clean cotton"] = Recipe("miller", (1, "lb"), [("cotton", 2.5)], [])
semiGoods.append("clean cotton")

recipeStorage["yarn, cotton"] = Recipe(
    "spinner",
    (1, "lb"),
    [],
    [("clean cotton", 1)],
...
# the weight calculation below works because the table lists 1 lb of yarn.
cottonClothWeight = (yarnFtPerClothSqFt / getUnitSize("yarn, cotton")) * getWeight(
    "yarn, cotton"
)
recipeStorage["cotton cloth"] = Recipe(
    "weaver",
    (cottonClothWeight, "lb"),
    [],
    [("yarn, cotton", yarnFtPerClothSqFt / getUnitSize("yarn, cotton"))],
    unit=(1, "sq ft"),
)

recipeStorage["thread, cotton"] = Recipe(
    "spinner",
    (1, "lb"),
    [],
    [("clean cotton", 1)],
...
recipeStorage["embroidery"] = Recipe(
    "embroiderer",
    (embroideryWeight, "lb"),
    [],
    [("thread", threadSqFtUR)],
...
recipeStorage["chemise"] = Recipe(
    "tailor",
    (chemiseWeight, "lb"),
    [],
    [("wool cloth", chemiseSqFt), ("thread", chemiseThread / getUnitSize("thread"))],
...
recipeStorage["tunic"] = Recipe(
    "tailor",
    (tunicWeight, "lb"),
    [],
    [("wool cloth", tunicSqFt), ("thread", tunicThread / getUnitSize("thread"))],
...
recipeStorage["chemise, fancy"] = Recipe(
    "tailor",
    (getWeight("chemise") + fancyChemiseTotalFrillWt, "lb"),
    [],
...
recipeStorage["brown sugar"] = Recipe(
    "miller", (1, "lb"), [("sugarcane", sugarcaneForPoundSugar)], [], difficulty=2
)

recipeStorage["molasses"] = Recipe(
    "miller",
    (molassesGallonWeight, "lb"),
...
recipeStorage["pork"] = Recipe(
    "butcher", (1, "lb"), [], [("pig", Decimal(1 / pigDressWeight))]
)
...
recipeStorage["beef jerky"] = Recipe(
    "butcher",
    (1, "lb"),
    [("salt", saltForSaltBeef)],
    [("brown sugar", brownSugarForSaltBeef), ("beef", 1)],
...
recipeStorage["salt pork"] = Recipe(
    "butcher",
    (1, "lb"),
    [("salt", saltForSaltPork)],
    [("brown sugar", brownSugarForSaltPork), ("pork", 1)],
    description="will keep for a year if seldom exposed to air",
)

recipeStorage["spearhead"] = Recipe(
    "blacksmith",
    (1.5, "lb"),
    [],
...
recipeStorage["spear haft"] = Recipe(
    "carpenter", (spearHaftCuFt * densityTimber, "lb"), [("timber", spearHaftCuFt)], []
)
semiGoods.append("spear haft")

spearWeight = (
    recipeStorage["spearhead"].weight[0] + recipeStorage["spear haft"].weight[0]
)
recipeStorage["spear"] = Recipe(
    "blacksmith",
    (spearWeight, "lb"),
    [],
...
recipeStorage["mace haft"] = Recipe(
    "blacksmith",
    (maceHaftMetalWeight + maceHaftWoodWeight, "lb"),
    [("timber", maceHaftCuFt)],
...
recipeStorage["mace flange"] = Recipe(
    "blacksmith", (maceFlangeWeight, "lb"), [], [("wrought iron", maceFlangeWeight)]
)
semiGoods.append("mace flange")

recipeStorage["mace"] = Recipe(
    "blacksmith",
    (getWeight("mace haft") + (6 * maceFlangeWeight), "lb"),
    [],
...
recipeStorage["mail ring"] = Recipe(
    "blacksmith", (unitsWirePerRing, "lb"), [], [("wire", unitsWirePerRing)]
)
...
recipeStorage["mail sqft"] = Recipe(
    "blacksmith",
    (getWeight("mail ring") * ringsInSquareFootMail, "lb"),
    [],
...
hauberkSqFt = 9
recipeStorage["hauberk, mail"] = Recipe(
    "blacksmith",
    (hauberkSqFt * getWeight("mail sqft"), "lb"),
    [],
...
haubergeonSqFt = 6
recipeStorage["haubergeon, mail"] = Recipe(
    "blacksmith",
    (haubergeonSqFt * getWeight("mail sqft"), "lb"),
    [],
...
recipeStorage["backpack"] = Recipe(
    "leatherworker",
    (backpackLeatherWeight + backpackYarnWt, "lb"),
    [],
    [("tanned cowhide", backpackPortionOfCowhide), ("yarn, wool", backpackYarnUR)],
...
recipeStorage["belt pouch"] = Recipe(
    "leatherworker",
    (beltpouchPortionOfCowhide * getWeight("tanned cowhide"), "lb"),
    [],
    [
...
recipeStorage["belt"] = Recipe(
    "leatherworker",
    (beltPortionOfCowhide * getWeight("tanned cowhide"), "lb"),
    [],
    [
...
recipeStorage["recorder"] = Recipe(
    "carpenter",
    (recorderBodyWeight + fippleWeight, "lb"),
...
recipeStorage["rope strand"] = Recipe(
    "ropewalker",
    (getWeight("yarn, wool"), "lb"),
    [],
    [("yarn, wool", 1)],
...
recipeStorage["rope"] = Recipe(
    "ropewalker",
    (getWeight("rope strand") * numberOfSegments, "lb"),
    [],
    [("rope strand", numberOfSegments)],
    unit=(
        (getUnitSize("rope strand") / strandFeetPerSegmentOfRope) * numberOfSegments,
        "feet",
    ),
    description="",
)
...
recipeStorage["felt"] = Recipe(
    "fuller",
    (poundsCleanWoolPerSqFtFelt, "lb"),
    [],
...
recipeStorage["gambeson, quilted"] = Recipe(
    "weaver",
    (gambesonSqFt * getWeight("wool cloth"), "lb"),
    [],
    [
        ("wool cloth", gambesonSqFt),
        ("thread", (gambesonThread / getUnitSize("thread"))),
    ],
...
gemshornRequiredHornWeightRatio = (gemshornRequiredHornCuFt * densityHorn) / getWeight(
    "horn, cow"
)
recipeStorage["gemshorn"] = Recipe(
    "carpenter",
    (2, "lb"),
    [],
    [("fipple", 1), ("horn, cow", gemshornRequiredHornWeightRatio)],
    description="ocarina-type instrument made from bull horn; "
    + str(gemshornRequiredHornLength)
    + " inches long",
)
...
recipeStorage["handbell handle"] = Recipe(
    "leatherworker",
    (handbellHandleWeight, "lb"),
    [],
...
recipeStorage["handbell clapper"] = Recipe(
    "blacksmith",
    (handbellClapperWeight, "lb"),
    [],
...
recipeStorage["handbell body"] = Recipe(
    "blacksmith", (handbellBodyWeight, "lb"), [], [("bell metal", handbellBodyWeight)]
)
semiGoods.append("handbell body")

recipeStorage["handbell"] = Recipe(
    "blacksmith",
    (handbellHandleWeight + handbellClapperWeight + handbellBodyWeight, "lb"),
    [],
...
recipeStorage["handaxe haft"] = Recipe(
    "carpenter", (handaxeHaftWeight, "lb"), [("timber", handaxeHaftCuFt)], []
)
...
recipeStorage["handaxe"] = Recipe(
    "blacksmith",
    (handaxeHeadWeight + handaxeHaftWeight, "lb"),
    [],
...
halberdHeadWeight = (
    2 * handaxeHeadWeight
)  # is wider and takes up more length along the haft
halberdTotalWeight = halberdHaftWeight + halberdHeadWeight
recipeStorage["halberd"] = Recipe(
    "blacksmith",
    (halberdTotalWeight, "lb"),
    [],
    [("spear haft", halberdHaftWeight), ("steel", halberdHeadWeight)],
    description="1d8 dmg; 2hand; "
    + str(halberdLength)
    + " ft long. Attacks normally; enemies 2 hexes away are threatened",
)

quarterstaffCuFt = cylinderCuFt(5, Decimal(0.5 / 12))
recipeStorage["quarterstaff"] = Recipe(
    "carpenter",
    (quarterstaffCuFt * densityTimber, "lb"),
...
javelinWeight = recipeStorage["spearhead"].weight[0] + (
    recipeStorage["spear haft"].weight[0] * ratioJavelinWeightSpearWeight
)
recipeStorage["javelin"] = Recipe(
    "blacksmith",
    (javelinWeight, "lb"),
    [],
    [("spear haft", ratioJavelinWeightSpearWeight), ("spearhead", 1)],
    description="1d4+1 damage; melee or thrown 6/9/12; "
    + str(ratioJavelinWeightSpearWeight * 5)
    + " ft long",
)
...
recipeStorage["sling"] = Recipe(
    "leatherworker",
    (slingLeatherWeight + slingYarnWeight, "lb"),
    [],
...
recipeStorage["musical bones"] = Recipe(
    "carpenter",
    (musicalBonePairCuFt * densityTimber, "lb"),
...
recipeStorage["leather lamella"] = Recipe(
    "leatherworker",
    (leatherLamellaWeight, "lb"),
    [],
...
leatherArmorWeight = (
    numberOfLeatherLamella * leatherLamellaWeight
) + leatherArmorYarnWeight
recipeStorage["leather armor"] = Recipe(
    "leatherworker",
    (leatherArmorWeight, "lb"),
    [],
    [
        ("leather lamella", numberOfLeatherLamella),
        ("yarn, wool", leatherArmorYarnWeight),
    ],
...
recipeStorage["shield, round wooden"] = Recipe(
    "carpenter",
    (
        shieldTimberWeight
        + (shieldRopeStrapWeight * 2)
        + shieldHandleWeight
        + shieldEdgingWeight,
        "lb",
    ),
    ...
    description="improves AC by -1; wooden shield "
    + str(shieldRadius * 2)
    + " feet across. has handle and rope straps",
)
...
recipeStorage["paint, red/yellow"] = Recipe(
    "potter",
    (poundsPigmentQuartPaint + milkGallonWeight / Decimal(4), "lb"),
    [],
    [("cow milk", 0.25), ("pigment, red/yellow", poundsPigmentQuartPaint)],
    unit=(1, "quart"),
    description="in powder form; covers "
    + str(squareFeetPaintCanCover)
    + " sq ft. Keeps for 6 mo. sealed or 4 days once once opened",
)

recipeStorage["paint, ultramarine blue"] = Recipe(
    "potter",
    (poundsPigmentQuartPaint + milkGallonWeight / Decimal(4), "lb"),
    [],
...
recipeStorage["sling bullet"] = Recipe(
    "potter",
    (slingBulletWeight, "lb"),
    [],
...
recipeStorage["sling bullet, inscribed"] = Recipe(
    "potter",
    (slingBulletWeight, "lb"),
    [],
    [("baked clay", slingBulletWeight)],
...
recipeStorage["pitch, bulk"] = Recipe(
    "potter",
    (weightWaterOneGal, "lb"),
...
recipeStorage["hat, bicorne"] = Recipe(
    "hatter",
    (bicorneWeight, "lb"),
    [],
...
recipeStorage["hat, tricorne"] = Recipe(
    "hatter",
    (tricorneWeight, "lb"),
    [],
...
recipeStorage["cup, earthenware"] = Recipe(
    "potter",
    (cupClayWeight, "lb"),
    [],
...
recipeStorage["cup, glass, clear"] = Recipe(
    "glassblower",
    (cupClayWeight, "lb"),
    [],
...
flaskOuterVolume = squarePyramidCuFt(
    Decimal(3.25) / Decimal(12), Decimal(5.25) / Decimal(12)
)
flaskCuFt = flaskOuterVolume - flaskInnerVolume
flaskClayWeight = flaskCuFt * densityClay
flaskStopperCuFt = cylinderCuFt(
    Decimal(0.75) / Decimal(12), Decimal(0.125) / Decimal(12)
)
recipeStorage["flask, earthenware"] = Recipe(
    "potter",
    (flaskClayWeight, "lb"),
...
recipeStorage["flask, glass"] = Recipe(
    "glassblower",
    (flaskClayWeight, "lb"),
    [("timber", flaskStopperCuFt)],
...
jarOuterVolume = cylinderCuFt(
    jarOuterHeight / Decimal(12), jarOuterRadius / Decimal(12)
)
# sphere part of lid is a little knob for lifting it out
jarLidCuFt = cylinderCuFt(
    Decimal(0.5) / Decimal(12), jarOuterRadius / Decimal(12)
) + sphereCuFt(Decimal(0.125) / Decimal(12))
jarCuFt = jarOuterVolume - jarInnerVolume + jarLidCuFt
jarClayWeight = jarCuFt * densityClay
recipeStorage["jar, earthenware"] = Recipe(
    "potter",
    (jarClayWeight, "lb"),
    [],
...
recipeStorage["pitch"] = Recipe(
    "potter",
    (pitchInFlaskWeightPitch + getWeight("flask, earthenware"), "lb"),
    [],
...
recipeStorage["button, ceramic"] = Recipe(
    "potter",
    (buttonClayWeight, "lb"),
    [],
...
recipeStorage["button, glass"] = Recipe(
    "glassblower",
    (buttonClayWeight, "lb"),
    [],
    [
        ("baked clay", buttonGlassWeightClay * 24),
...
recipeStorage["waistcoat"] = Recipe(
    "tailor",
    (waistcoatWeightWool + waistcoatThreadWeight, "lb"),
    [],
    [
        ("wool cloth", waistcoatSqFtWool),
        ("thread", waistcoatThreadUnitRatio),
        ("button, ceramic", 0.5),
    ],
...
recipeStorage["dress"] = Recipe(
    "tailor",
    (dressWeightWool + dressThreadWeight, "lb"),
    [],
...
recipeStorage["cowl"] = Recipe(
    "tailor",
    (cowlThreadWt + cowlClothWt, "lb"),
    [],
    [("wool cloth", cowlSqFt), ("thread", cowlThreadUR)],
...
recipeStorage["cassock"] = Recipe(
    "tailor",
    (cowlThreadWt + cowlClothWt, "lb"),
    [],
    [("cowl", 1), ("button, ceramic", 1)],
    difficulty=2,
...
scapularWt = (scapularFinalClothSqFt / getUnitSize("wool cloth")) * getWeight(
    "wool cloth"
)
recipeStorage["scapular"] = Recipe(
    "tailor",
    (scapularWt, "lb"),
    [],
...
skirtSqFtWool = 5
skirtThread = (
    4  # one 2-foot thread connecting it together like a tube, doubled for strength
)
...
recipeStorage["skirt"] = Recipe(
    "tailor",
    (skirtWeightWool + skirtThreadWeight, "lb"),
    [],
...
recipeStorage["bodice"] = Recipe(
    "tailor",
    (bodiceWeightWool + bodiceThreadWeight + bodiceLacingWeight, "lb"),
    [],
    [
...
recipeStorage["knit cap"] = Recipe(
    "tailor",
    (knitCapWeightWool, "lb"),
    [],
...
recipeStorage["stockings"] = Recipe(
    "tailor",
    (stockingWeightWool * stockingNumberSold, "lb"),
    [],
...
breechLegSqFt = truncatedConeCuFt(
    Decimal(2), Decimal(1.5), Decimal(1.25)
) - truncatedConeCuFt(Decimal(1.8), Decimal(1.3), Decimal(1.25))
...
recipeStorage["breeches"] = Recipe(
    "tailor",
    (breechesWoolWeight, "lb"),
    [],
    [
        ("wool cloth", breechesWoolSqFt),
        ("thread", breechesThreadWeight),
        ("button, ceramic", 0.75),
    ],  # uses 18 buttons
...
recipeStorage["shoe"] = Recipe(
    "cobbler",
    (shoeLeatherWeight + shoeHeelTimberWeight, "lb"),
...
recipeStorage["mace, masterwork"] = Recipe(
    "blacksmith",
    (getWeight("mace haft") + (6 * maceFlangeWeight), "lb"),
    [],
    [("mace haft", 1), ("mace flange", 6)],
...
recipeStorage["aqua fortis"] = Recipe(
    "alchemist",
    (aquaFortisWeight + getWeight("flask, glass"), "lb"),
    [("copper ore", 3)],
    [],
    unit=(8, "fl oz"),
...
chickenFeedLbs = 6 * 4
recipeStorage["chicken, mature"] = Recipe(
    "farmer",
    (5, "lb"),
...
recipeStorage["chicken, killed"] = Recipe(
    "butcher",
    (5, "lb"),
    [],
    [("chicken, mature", 1)],
...
recipeStorage["chicken, butchered"] = Recipe(
    "butcher",
    (chickenDressPercentage * getWeight("chicken, killed"), "lb"),
    [],
    [("chicken, killed", 1)],
...
recipeStorage["feathers, chicken"] = Recipe(
    "butcher",
    (feathersPercentageOfChickenWeight * getWeight("chicken, killed"), "lb"),
    [],
...
recipeStorage["egg, chicken"] = Recipe(
    "farmer",
    (Decimal(0.125), "lb"),
    [],
    [
        ("chicken, mature", 1 / eggsChickenPerYear),
        ("feed, cattle/horse", egglayingChickenFeedLbs / eggsChickenPerYear),
    ],
)

# base volume of starting material: 6 x 2 x 2 inches
statuetteCuFt = (
    (Decimal(6) / Decimal(12)) * (Decimal(2) / Decimal(12)) * (Decimal(2) / Decimal(12))
)
# let's assume 1/4 of the material is carved away
statuetteWoodWeight = Decimal(0.75) * (statuetteCuFt * densityTimber)
recipeStorage["statuette, wooden, small"] = Recipe(
    "carver",
    (statuetteWoodWeight, "lb"),
...
recipeStorage["statue, wooden, life-size"] = Recipe(
    "carver",
    (statueWoodWeight, "lb"),
    [("timber", statueCuFt)],
    [],
...
gongSmallTotalCuFt = (
    gongSmallBodyCuFt
    + gongSmallButtonCuFt
    - gongSmallIntersectionCuFt
    - (2 * gongHoleCuFt)
)
gongSmallWeight = gongSmallTotalCuFt * densityBronze
recipeStorage["gong, small"] = Recipe(
    "blacksmith",
    (gongSmallWeight, "lb"),
    [],
    [("bronze", gongSmallWeight)],
    description="bronze button gong; "
    + str(2 * gongSmallRadius)
    + " feet across; with 2 holes for hanging",
)
...
gongSmallFrameLongPiece = gongSmallFrameCommonElement * Decimal(
    1.5 * (2 * gongSmallRadius)
)
...
recipeStorage["gong frame, small, plain"] = Recipe(
    "carpenter",
    (gongSmallFrameWeight, "lb"),
...
recipeStorage["torch stake"] = Recipe(
    "carver",
    (torchStakeWeight, "lb"),
...
recipeStorage["torch fuel"] = Recipe(
    "carver",
    (torchFuelWoolWeight + pitchInFlaskWeightPitch, "lb"),
    [],
...
recipeStorage["spike"] = Recipe(
    "blacksmith",
    (spikeWeight, "lb"),
    [],
    [("wrought iron", spikeWeight)],
    description="for affixing ropes and chains; "
    + str(spikeLength)
    + " ft long, "
    + str(2 * spikeRadius)
    + " in. thick",
...
recipeStorage["cake, sponge"] = Recipe(
    "baker",
    (cakeSpongeWeightEggs * 4, "lb"),
    [],
    [
        ("egg, chicken", 3),
        ("brown sugar", cakeSpongeWeightEggs),
        ("flour", cakeSpongeWeightEggs),
        ("suet", cakeSpongeWeightEggs),
    ],
...
recipeStorage["tankard"] = Recipe(
    "potter",
    (clayTankardWeight, "lb"),
    [],
    [("baked clay", clayTankardWeight)],
    description="earthenware, "
    + str(tankardOuterHeight * 12)
    + " in. tall; holds "
    + str(tankardInnerVolumeInPints)
    + " pints",
)
...
chaliceBaseCuFt = truncatedConeCuFt(
    chaliceBaseBigRadius, chaliceBaseSmallRadius, chaliceBaseHeight
)
...
recipeStorage["chalice"] = Recipe(
    "tinsmith",
    (leadChaliceWeight, "lb"),
    [("lead ore", leadChaliceWeight)],
    [],
    description="lead; "
    + str(chaliceTotalHeight * 12)
    + " inches tall; holds "
    + str(chaliceBowlInnerVolumePints)
    + " pints",
)
...
recipeStorage["goblet"] = Recipe(
    "tinsmith",
    (leadGobletWeight, "lb"),
    [("lead ore", leadGobletWeight)],
    [],
    description="long-stemmed chalice; " + str(gobletTotalHeight * 12) + " inches tall; same capacity as chalice",
)


# TODO: bricks need straw or flax in them to make them strong!!!
brickSqFt = (
    Decimal(4) / Decimal(12) * Decimal(8) / Decimal(12) * Decimal(2) / Decimal(12)
)
brickWeight = brickSqFt * densityClay
recipeStorage["brick"] = Recipe(
    "potter", (brickWeight, "lb"), [], [("baked clay", brickWeight)]
)

spiritMashWaterGal = gallonsPerBarrel
spiritMashBrownSugarLbs = 6 * spiritMashWaterGal
desiredVolumeSpiritGal = gallonsPerBarrel
spiritDilutedABV = calculateABV(
    spiritMashBrownSugarLbs, 0, 0, spiritMashWaterGal, desiredVolumeSpiritGal
)
recipeStorage["rum, barrel"] = Recipe(
    "brewer",
    (weightWaterOneGal * gallonsPerBarrel + getWeight("barrel"), "lb"),
    [],
...
recipeStorage["rum, flask"] = Recipe(
    "brewer",
    (flaskRumWeight, "lb"),
    [],
    [("rum, barrel", Decimal(0.5) / pintsPerBarrel), ("flask, glass", 1)],
    unit=(8, "fl oz"),
    description="light rum; in half-pint glass flask; "
    + str(spiritDilutedABV)
    + "% alcohol",
)


recipeStorage["tobacco, uncured"] = Recipe("farmer", (1, "lb"), [("tobacco", 1)], [])
semiGoods.append("tobacco, uncured")

recipeStorage["tobacco, cured"] = Recipe(
    "farmer", (1, "lb"), [], [("tobacco, uncured", 5)]
)
...
recipeStorage["pipe tobacco"] = Recipe(
    "tobacconist",
    (pipeTobaccoWeight + clothTwistSqFt, "lb"),
    [],
...
recipeStorage["pipe, smoking"] = Recipe(
    "potter",
    (pipeSmokingWeight, "lb"),
    [],
    [("baked clay", pipeSmokingWeight)],
    description="long-stemmed tobacco pipe",
)

recipeStorage["herring, fresh"] = Recipe("fishmonger", (1, "lb"), [("herring", 1)], [])
semiGoods.append("herring, fresh")

recipeStorage["cod, fresh"] = Recipe("fishmonger", (1, "lb"), [("cod", 1)], [])
...
recipeStorage["breastplate"] = Recipe(
    "blacksmith",
    (breastplateWeight, "lb"),
    [],
...
recipeStorage["rerebrace"] = Recipe(
    "blacksmith",
    (rerebraceWeight, "lb"),
    [],
...
halfPlateWeight = (
    breastplateWeight + 2 * pauldronWeight + 2 * rerebraceWeight + 2 * vambraceWeight
)
fullPlateWeight = halfPlateWeight + fauldWeight + 2 * cuissWeight + 2 * greaveWeight

recipeStorage["half-plate armor"] = Recipe(
    "blacksmith",
    (halfPlateWeight, "lb"),
    [],
...
recipeStorage["full-plate armor"] = Recipe(
    "blacksmith",
    (fullPlateWeight, "lb"),
    [],
    recipeStorage["half-plate armor"].subRecipes
    + [("fauld", 1), ("cuiss", 2), ("greave", 2)],
...
recipeStorage["mercury"] = Recipe(
    "smelter",
    (weightMercuryHalfPint, "lb"),
    [("cinnabar", cinnabarForHalfPintMercury)],
    [
        ("quicklime", cinnabarForHalfPintMercury)
    ],  # following the method presented in Subterraneal Treasure: use as much lime as cinnabar
    unit=(0.5, "pint"),
...
coatOfPlatesSteelWeight = coatOfPlatesSteelSqFt * steelPlatingWeight(
    coatOfPlatesSteelThickness
)
...
recipeStorage["coat of plates"] = Recipe(
    "leatherworker",
    (coatOfPlatesTotalWeight, "lb"),
    [],
...
recipeStorage["arrow shaft"] = Recipe(
    "carver", (arrowShaftWeight, "lb"), [("timber", arrowShaftCuFt)], []
)
...
recipeStorage["arrowhead"] = Recipe(
    "blacksmith", (arrowheadWeight, "lb"), [], [("steel", arrowheadWeight)]
)
...
arrowTotalWeight = (
    getWeight("arrow shaft") + getWeight("arrowhead") + arrowFeathersWeight
)
# for now, I'll use the bowyer for both bowmaking and fletching recipes
recipeStorage["arrow"] = Recipe(
    "bowyer",
    (arrowTotalWeight, "lb"),
    [],
    [("feathers, chicken", arrowFeathersWeight), ("arrow shaft", 1), ("arrowhead", 1)],
...
squareFeetGlueCanCover = (
    hideGlueDesiredWeight
    / getWeight("paint, ultramarine blue")
    * squareFeetPaintCanCover
)
recipeStorage["hide glue"] = Recipe(
    "tanner",
    (hideGlueDesiredWeight, "lb"),
    [],
    [("rawhide", hideGlueWeightRatio)],
    description="customer supplies container; covers "
    + str(squareFeetGlueCanCover)
    + " sq ft; heat 1 pt glue in 2 pts water to use",
)

recipeStorage["gelatin"] = Recipe(
    "tanner",
    (1, "lb"),
    [],
    [("hide glue", 1)],
...
bowHeight = Decimal(5)
bowstringLength = bowHeight * Decimal(
    1.10
)  # extra is for tying knots at each end of the bow
...
recipeStorage["bowstring strand"] = Recipe(
    "leatherworker",
    (bowstringWeight / strandsPerBowstring, "lb"),
    [],
    [("rawhide", bowstringUnitRatio / strandsPerBowstring)],
)
semiGoods.append("bowstring strand")

recipeStorage["bowstring"] = Recipe(
    "leatherworker",
    (bowstringWeight, "lb"),
    [],
...
recipeStorage["bow limb wood section"] = Recipe(
    "bowyer", (bowLimbWoodWeight, "lb"), [("timber", bowLimbWoodSectionCuFt)], []
)
...
recipeStorage["bow limb horn section"] = Recipe(
    "carver", (bowLimbHornWeight, "lb"), [], [("horn, cow", bowLimbHornRatio)]
)
...
recipeStorage["bow riser"] = Recipe(
    "blacksmith",
    (bowRiserWeight, "lb"),
    [],
...
recipeStorage["bow"] = Recipe(
    "bowyer",
    (bowTotalWeight, "lb"),
    [],
    [("bowstring", 1), ("bow riser", 1), ("bow limb", 2)],
    description="1d6 damage; range 12/24/36; "
    + str(bowHeight)
    + " ft tall; includes 1 bowstring",
)
...
recipeStorage["ladder rung"] = Recipe(
    "carver", (ladderRungWeight, "lb"), [("timber", ladderRungCuFt)], []
)
...
ropeLadderUnitRopeFt = (
    ropeLadderUnitBridgingRopeLength + ropeLadderUnitSecuringRopeLength
)
...
recipeStorage["rope ladder tophook"] = Recipe(
    "blacksmith",
    (ropeLadderTophookWeight, "lb"),
    [],
...
ropeLadderTotalRopeUnitRatio = (
    ropeLadderUnitRopeRatio * ropeLadderUnitCount + ropeLadderHookRopeUnitRatio
)
ropeLadderTotalWeight = (
    (ropeLadderUnitRopeWeight * ropeLadderUnitCount)
    + (ladderRungWeight * (ropeLadderUnitCount + 1))
    + (ropeLadderTophookWeight * 2)
    + (ropeLadderHookRopeWeight * 2)
)
recipeStorage["ladder, rope"] = Recipe(
    "ropewalker",
    (ropeLadderTotalWeight, "lb"),
    [],
    [
    ...
    description="rollable; 2x hook -> no need to spike; as hard to climb as rope due to swinging; body height: "
    + str(ropeLadderBodyLength(ropeLadderUnitCount))
    + " ft",
)
...
woodenLadderPlankCuFt = (
    woodenLadderHeight * Decimal(2) / Decimal(12) * Decimal(2) / Decimal(12)
)
woodenLadderPlankWeight = woodenLadderPlankCuFt * densityTimber
recipeStorage["wooden ladder plank"] = Recipe(
    "carpenter",
    (woodenLadderPlankWeight, "lb"),
    [("timber", woodenLadderPlankCuFt)],
    [],
)
semiGoods.append("wooden ladder plank")

woodenLadderTotalWeight = (woodenLadderPlankWeight * 2) + (
    ladderRungWeight * woodenLadderNumRungs
)
recipeStorage["ladder, wooden"] = Recipe(
    "carpenter",
    (woodenLadderTotalWeight, "lb"),
    [],
    [("wooden ladder plank", 2), ("ladder rung", woodenLadderNumRungs)],
...
recipeStorage["fishing line"] = Recipe(
    "weaver",
    (fishingLineWeight, "lb"),
    [],
...
recipeStorage["fish hook"] = Recipe(
    "blacksmith", (fishHookWeight, "lb"), [], [("wrought iron", fishHookWeight)]
)
...
fishingRodTipLoopCuFt = cylinderCuFt(
    fishingRodTipLoopLength, fishingRodTipLoopDiameter / Decimal(2)
)
...
recipeStorage["fishing rod"] = Recipe(
    "carver",
    (fishingRodTotalWeight, "lb"),
    [("timber", fishingRodCuFt)],
    [("wrought iron", fishingRodTipLoopWeight)],
    description="for angling; "
    + str(fishingRodLength)
    + " feet long; breaks down into 3 6-fot sections (dis/assembly time 1 minute)",
)
...
innDailyCoal = Decimal(
    10
)  # lbs, a TOTAL guess, I have still not figured out coal heating values at all
...
recipeStorage["room at inn, communal"] = Recipe(
    "innkeeper",
    (0, "lb"),
    [],
    [
        ...
        (
            "beer, barrel",
            innTotalEmployeeDailyBeer
            / innMainRoomMaxPatrons
            * Decimal(1)
            / pintsPerBarrel,
        ),
    ],
...
recipeStorage["egg oil"] = Recipe(
    "alchemist",
    ((waterWeightOnePint / Decimal(2)) + getWeight("flask, glass"), "lb"),
    [],
...
recipeStorage["flax, dried"] = Recipe("farmer", (1, "lb"), [("flax", 4)], [])
...
recipeStorage["flax, retted"] = Recipe(
    "miller",
    (1, "lb"),
    [],
...
recipeStorage["flax, beaten"] = Recipe(
    "miller",
    (1, "lb"),
    [],
...
foolscapLeafPoundsPerSqFt = (
    ratioFlaxPaper
    / ((foolscapHeight / Decimal(12)) * (foolscapLength / Decimal(12)))
    / foolscapReamLeaves
)
# works out to 3.5 grams per leaf, that's not bad
recipeStorage["paper, foolscap, unfinished"] = Recipe(
    "miller",
    (foolscapReamWeight, "lb"),
    [],
...
recipeStorage["paper, foolscap, sized"] = Recipe(
    "miller",
    (1, "lb"),
    [],
    [("paper, foolscap, unfinished", 1), ("gelatin", gelatinPerReam)],
...
recipeStorage["paper, foolscap, looseleaf"] = Recipe(
    "miller",
    (1, "lb"),
    [],
...
recipeStorage["paper, foolscap, quire"] = Recipe(
    "bookbinder",
    (quireWeight, "lb"),
    [],
    [
...
cigPaperUnitRatio = cigPaperSqFt / (
    getUnitSize("paper, foolscap, unfinished") * foolscapHeight * foolscapLength
)
...
recipeStorage["cigar"] = Recipe(
    "tobacconist",
    (cigTobaccoLb + cigPaperWeight, "lb"),
    [],
    [
...
recipeStorage["green vitriol"] = Recipe(
    "alchemist",
    (1, "lb"),
    [("iron ore", 4)],
    [],
...
recipeStorage["timber, ground"] = Recipe(
    "miller",
    (1, "lb"),
...
recipeStorage["tannin"] = Recipe(
    "dyer", (tanninOutputWeight, "lb"), [], [("timber, ground", tanninWoodCuFt)]
)
...
inkBlackSolidWeight = (
    inkBlackPoundsTanninPerPint
    + inkBlackPoundsGelatinPerPint
    + inkBlackPoundsVitriolPerPint
)
...
inkBlackLiquidWeight = (
    (inkBlackWaterPerPint * waterWeightOnePint)
    + (inkBlackAlePerPint * waterWeightOnePint)
    + (inkBlackVinegarPerPint * weightPintVinegar)
)
inkBlackTotalWeight = inkBlackSolidWeight + inkBlackLiquidWeight
inkSaleUnit = Decimal(0.5)
recipeStorage["ink, black"] = Recipe(
    "dyer",
    ((inkBlackTotalWeight * inkSaleUnit) + getWeight("flask, glass"), "lb"),
    [],
    [
...
recipeStorage["ink, ultramarine blue"] = Recipe(
    "dyer",
    (inkColorTotalWeight + getWeight("flask, glass"), "lb"),
    [],
    [
...
recipeStorage["ink, red/yellow"] = Recipe(
    "dyer",
    (inkColorTotalWeight + getWeight("flask, glass"), "lb"),
    [],
    [
...
recipeStorage["tattoo needle"] = Recipe(
    "blacksmith",
    (tattooNeedleWeight, "lb"),
    [],
...
for service in allServiceNames:
    recipeStorage["hireling, " + service] = Recipe(
        service,
        (0, "lb"),
...
recipeStorage["hireling, mercenary"] = Recipe(
    "laborer",
    (0, "lb"),
    [],
    [("hireling, laborer", mercenaryPersonValue)]
    + [(weapon, Decimal(1) / paymentsPerYear) for weapon in basicWeapons],
    unit=(Decimal(1), "head"),
...
recipeStorage["hireling, fighter, level 1"] = Recipe(
    "laborer",
    (0, "lb"),
    [],
...
recipeStorage["caulk"] = Recipe(
    "shipwright",
    (caulkWeight + getWeight("flask, earthenware"), "lb"),
    [],
...
recipeStorage["rivet"] = Recipe(
    "coppersmith", (rivetWeight, "lb"), [], [("copper", rivetWeight)]
)
...
recipeStorage["keel, faering"] = Recipe(
    "shipwright", (faeringKeelWeight, "lb"), [("timber", faeringKeelCuFt)], []
)
...
faeringStemCuFtSupp = triangularPrismCuFt(
    faeringStemWidth, faeringKeelHeight - faeringStemHeight, faeringStemLength
)
faeringStemTotalCuFt = faeringStemCuFtMain + faeringStemCuFtSupp
faeringStemWeight = faeringStemTotalCuFt * densityTimber
recipeStorage["stem, faering"] = Recipe(
    "shipwright", (faeringStemWeight, "lb"), [("timber", faeringStemTotalCuFt)], []
)
semiGoods.append("stem, faering")

recipeStorage["stem, faering, shaped"] = Recipe(
    "shipwright", (getWeight("stem, faering"), "lb"), [], [("stem, faering", 1)]
)
...
faeringGarboardCuFt = (
    faeringGarboardLength * faeringGarboardWidth * faeringGarboardHeight
)
faeringGarboardWeight = faeringGarboardCuFt * densityTimber
recipeStorage["garboard, faering"] = Recipe(
    "shipwright", (faeringGarboardWeight, "lb"), [("timber", faeringGarboardCuFt)], []
)
semiGoods.append("garboard, faering")

recipeStorage["garboard, faering, shaped"] = Recipe(
    "shipwright", (getWeight("garboard, faering"), "lb"), [], [("garboard, faering", 1)]
)
...
faeringBilgestrakeCuFt = (
    faeringBilgestrakeLength * faeringBilgestrakeWidth * faeringBilgestrakeHeight
)
faeringBilgestrakeWeight = faeringBilgestrakeCuFt * densityTimber
recipeStorage["bilgestrake, faering"] = Recipe(
    "shipwright",
    (faeringBilgestrakeWeight, "lb"),
    [("timber", faeringBilgestrakeCuFt)],
    [],
)
semiGoods.append("bilgestrake, faering")

recipeStorage["bilgestrake, faering, shaped"] = Recipe(
    "shipwright",
    (getWeight("bilgestrake, faering"), "lb"),
    [],
...
faeringSheerstrakeCuFt = (
    faeringSheerstrakeLength * faeringSheerstrakeWidth * faeringSheerstrakeHeight
)
faeringSheerstrakeWeight = faeringSheerstrakeCuFt * densityTimber
recipeStorage["sheerstrake, faering"] = Recipe(
    "shipwright",
    (faeringSheerstrakeWeight, "lb"),
    [("timber", faeringSheerstrakeCuFt)],
    [],
)
semiGoods.append("sheerstrake, faering")

recipeStorage["sheerstrake, faering, shaped"] = Recipe(
    "shipwright",
    (getWeight("sheerstrake, faering"), "lb"),
    [],
...
recipeStorage["gunwale, faering"] = Recipe(
    "shipwright", (faeringGunwaleWeight, "lb"), [("timber", faeringGunwaleCuFt)], []
)
semiGoods.append("gunwale, faering")

recipeStorage["gunwale, faering, shaped"] = Recipe(
    "shipwright", (getWeight("gunwale, faering"), "lb"), [], [("gunwale, faering", 1)]
)
...
faeringMidshipFrameTimberCuFt = (
    faeringMidshipFrameTimberLength * faeringFrameTimberHeight * faeringFrameTimberWidth
)
faeringMidshipFrameTimberWeight = faeringMidshipFrameTimberCuFt * densityTimber
recipeStorage["midship frame timber, faering"] = Recipe(
    "shipwright",
    (faeringMidshipFrameTimberWeight, "lb"),
...
faeringAftFrameTimberCuFt = (
    faeringAftFrameTimberLength * faeringFrameTimberHeight * faeringFrameTimberWidth
)
faeringAftFrameTimberWeight = faeringAftFrameTimberCuFt * densityTimber
recipeStorage["aft frame timber, faering"] = Recipe(
    "shipwright",
    (faeringAftFrameTimberWeight, "lb"),
...
faeringForeFrameTimberCuFt = (
    faeringForeFrameTimberLength * faeringFrameTimberHeight * faeringFrameTimberWidth
)
faeringForeFrameTimberWeight = faeringForeFrameTimberCuFt * densityTimber
recipeStorage["fore frame timber, faering"] = Recipe(
    "shipwright",
    (faeringForeFrameTimberWeight, "lb"),
...
faeringRongeFrameTimberCuFt = (
    faeringRongeFrameTimberLength * faeringFrameTimberHeight * faeringFrameTimberWidth
)
faeringRongeFrameTimberWeight = faeringRongeFrameTimberCuFt * densityTimber
recipeStorage["ronge frame timber, faering"] = Recipe(
    "shipwright",
    (faeringRongeFrameTimberWeight, "lb"),
...
faeringMidshipFlooringCuFt = (
    faeringFlooringHeight
    * faeringMidshipFlooringLength
    * faeringMidshipFrameTimberLength
)
faeringMidshipFlooringWeight = faeringMidshipFlooringCuFt * densityTimber
recipeStorage["midship flooring, faering"] = Recipe(
    "shipwright",
    (faeringMidshipFlooringWeight, "lb"),
...
faeringForeFlooringCuFt = triangularPrismCuFt(
    faeringForeFrameTimberLength, faeringForeFlooringLength, faeringFlooringHeight
)
faeringForeFlooringWeight = faeringForeFlooringCuFt * densityTimber
recipeStorage["fore flooring, faering"] = Recipe(
    "shipwright",
    (faeringForeFlooringWeight, "lb"),
...
faeringAftFlooringCuFt = triangularPrismCuFt(
    faeringAftFrameTimberLength, faeringAftFlooringLength, faeringFlooringHeight
)
faeringAftFlooringWeight = faeringAftFlooringCuFt * densityTimber
recipeStorage["aft flooring, faering"] = Recipe(
    "shipwright",
    (faeringAftFlooringWeight, "lb"),
...
faeringHullJoinLinearFeet = Decimal(2) * (
    faeringRongeFrameTimberLength
    + faeringKeelLength
    ...
    Decimal(2)
    * (
        faeringGarboardWidth
        + faeringBilgestrakeWidth
        + faeringSheerstrakeWidth
        + faeringGunwaleWidth
    )
)
faeringHullJoinLinearFeet += (
    faeringMidshipFrameTimberLength
    + faeringAftFrameTimberLength
    + faeringForeFrameTimberLength
)


faeringHullJoinSquareFeet = Decimal(2) * (
    (faeringKeelLength * faeringKeelHeight)
    + (faeringGarboardLength * faeringLapWidth)
    + (faeringBilgestrakeLength * faeringLapWidth)
    + (faeringSheerstrakeLength * faeringLapWidth)
)
faeringHullJoinSquareFeet += faeringFrameTimberWidth * (
    faeringMidshipFrameTimberLength
    + faeringForeFrameTimberLength
    + faeringAftFrameTimberLength
    + (Decimal(2) * faeringRongeFrameTimberLength)
)
...
faeringHullTotalWeight = (
    getWeight("keel, faering")
    + (Decimal(2) * getWeight("garboard, faering"))
    + (Decimal(2) * getWeight("bilgestrake, faering"))
    + (Decimal(2) * getWeight("sheerstrake, faering"))
    + (Decimal(2) * getWeight("bilgestrake, faering"))
    + (faeringHullRivets * getWeight("rivet"))
    + faeringHullCaulk
)
faeringHullTotalWeight += (
    getWeight("midship frame timber, faering")
    + getWeight("fore frame timber, faering")
    + getWeight("aft frame timber, faering")
    + (Decimal(2) * getWeight("ronge frame timber, faering"))
)
faeringHullTotalWeight += (
    getWeight("aft flooring, faering")
    + getWeight("fore flooring, faering")
    + (Decimal(2) * getWeight("midship flooring, faering"))
)
recipeStorage["hull, faering"] = Recipe(
    "shipwright",
    (faeringHullTotalWeight, "lb"),
    [],
    [
...
recipeStorage["outfit, monk"] = Recipe(
    "tailor",
    (outfitMonkWt, "lb"),
    [],
    outfitMonkParts,
    description="contains: "
    + ", ".join([str(y) + "x " + x for x, y in outfitMonkParts]),
)
...
def outfit(parts):
    ...
    return Recipe("tailor", (weight, "lb"), [], parts, description="contains: " + desc)
