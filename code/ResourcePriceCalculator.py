...
for name in allResourceNames:
    averageResourceReferences[name] = originalWorldResourceCounts[name] / Decimal(
        len(list(originalTowns.keys()))
    )

averageServiceReferences = {}
for name in allServiceNames:
    averageServiceReferences[name] = originalWorldServiceCounts[name] / Decimal(
        len(list(originalTowns.keys()))
    )
...
goldRefPriceCP = (
    Decimal(8) * Decimal(16) * singleReferenceProduction["gold ore"][0] * 100
)
...
for t, d in towns.items():
    ...
    for rawMat in allResourceNames:
        ...
        localRefToWorldRefRatio = d.resources[rawMat] / (
            pseudoAverageRefPercent * originalWorldResourceCounts[rawMat]
        )
